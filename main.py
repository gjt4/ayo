...
import telebot
from config import TOKEN, admin, chat_bota, instruction, nicknameadm, procent, token_qiwi, admin2, chat_id_bot, instruction, chat_bota
...
def handler_call(call):
	...
	if str(chat_id_bot) in str(call.message.chat.id):
		pass
	else:
		if call.data == 'output':
			info = func.profile(user_id=chat_id)
		...
		elif call.data == 'bor' and chat_id == admin or chat_id == admin2:
			bot.send_message(chat_id, text='Что вы хотите сделать?', reply_markup=kb.bor)


		elif call.data == 'unban' and chat_id == admin or chat_id == admin2:
		...
		elif call.data == 'ban' and chat_id == admin or chat_id == admin2:
		...
		elif call.data == 'edit_balance' and chat_id == admin or chat_id == admin2:
		...
		elif call.data == 'statistics' and chat_id == admin or chat_id == admin2:
		...
		elif call.data == 'dispute_admin' and chat_id == admin or chat_id == admin2:
		...
		elif call.data == 'customer_true' and chat_id == admin or chat_id == admin2:
		...
		elif call.data == 'seller_true' and chat_id == admin or chat_id == admin2:
		...
		elif call.data == 'no_true' and chat_id == admin or chat_id == admin2:
			msg = bot.send_message(chat_id, text='Покупатель вернёт деньги, а сделка будет отменена!\nДля подтверждения введите ID сделки, для отмены введите "-" без кавычек.')
			bot.register_next_step_handler(msg, customer_true_func)
